...
Uses Pinecone for vector search with HuggingFace embeddings.
...
class EmbeddingClient:
    """Embedding client using HuggingFace embeddings."""
    ...
    def __init__(self, settings: Settings):
        ...
        logger.info(
            f"HuggingFace embedding client initialized with model: {self.model_name}"
        )
    ...
    def embed_text(self, text: str, is_query: bool = True) -> List[float]:
        ...
        Args:
            text: Text to embed
            is_query: Unused for HuggingFace but kept for compatibility
...
class VectorStore:
    ...
    def __init__(self, settings: Settings, embedding_client: EmbeddingClient):
        ...
        sample_embedding = self.embedding_client.embed_text(
            "sample",
            is_query=False
        )
        ...
        logger.info(
            f"Vector store initialized with Pinecone index: {self.index_name}"
        )
    ...
    def add_documents(self, documents: List[Document]) -> None:
        ...
        logger.info(
            f"Added {len(documents)} documents to Pinecone index"
        )
    ...
    def delete_all(self) -> None:
        ...
        logger.info(
            f"Deleted all vectors from index: {self.index_name}"
        )
...
def load_knowledge_base(
    ...
    try:
        ...
        logger.info(
            f"Loaded {len(documents)} document chunks into vector store"
        )
...
def format_retrieval_context(
    ...
    for i, doc in enumerate(
        retrieval_result.documents[:max_docs]
    ):
